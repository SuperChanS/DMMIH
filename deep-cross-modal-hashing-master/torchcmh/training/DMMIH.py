...
from torchcmh.utils import calc_neighbor, mmd_rbf1
...
class DMMIH(TrainBase):
    ...
    def object_function(self, hash_layers1, hash_layer2, final_hash1, final_hash2, label1, label2, F, G, ind1, ind2):
        ...
        inter_loss1r, inter_loss2r = calc_inter_lossr(hash_layers1, hash_layer2, S_inter1_r, S_inter2_r, G,
                                                   self.parameters['alpha'],self.bit)
        inter_lossr = 0.5 * (inter_loss1r + inter_loss2r)
        intra_loss1r, intra_loss2r = calc_intra_lossr(hash_layers1, hash_layer2, S_inter1_r, S_inter2_r, F,
                                                   self.parameters['alpha'],self.bit)
        intra_lossr = 0.5 * (intra_loss1r + intra_loss2r) * self.parameters['lambda']
        inter_loss = inter_lossh+inter_lossr
        intra_loss = intra_lossh+intra_lossr
...
def train(dataset_name: str, img_dir: str, bit: int, img_net_name='resnet34', visdom=True, batch_size=128, cuda=True, **kwargs):
    ...
    # img_net = img_networks()
    trainer = DMMIH(dataset_name, img_dir, bit, img_net, visdom, batch_size, cuda, **kwargs)
    trainer.train()
...
def calc_inter_lossr(hash1_layers, hash2_layers, S1, S2, O, alpha,bit):
    inter_loss1 = 0
    for index, hash1_layer in enumerate(hash1_layers):
        theta = 1.0 / alpha * torch.matmul(hash1_layer, O.t())
        logloss = alpha * torch.sum(torch.pow((theta+bit)/2-S1*bit, 2))
        if torch.isinf(logloss):
            print("the log loss is inf in hash1 of layer %d, with the max of theta is %3.4f" % (index, torch.max(theta).data))
        inter_loss1 += logloss
    inter_loss2 = 0
    for index, hash2_layer in enumerate(hash2_layers):
        theta = 1.0 / alpha * torch.matmul(hash2_layer, O.t())
        logloss = alpha * torch.sum(torch.pow((theta+bit)/2-S2*bit, 2))
        if torch.isinf(logloss):
            print("the log loss is inf in hash2 of layer %d, with the max of theta is %3.4f" % (index, torch.max(theta).data))
        inter_loss2 += logloss
    return inter_loss1 / len(hash1_layers), inter_loss2 / len(hash2_layers)
...
def calc_intra_lossr(hash1_layers, hash2_layers, S1, S2, O, alpha,bit):
    intra_loss1 = 0
    for hash in hash1_layers:
        theta = 1.0 / alpha * torch.matmul(hash, O.t())
        logloss = alpha * torch.sum(torch.pow((theta+bit)/2-S1*bit, 2))
